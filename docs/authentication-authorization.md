...
Menetap has the user-role model and authorization helpers in `app/convex/roles.ts` and `app/convex/authorization.ts`. Guest booking currently works without an account, which is intentional for the MVP funnel.
...
Do not mark authentication complete until the provider is selected, local sign-in works, a protected Convex query rejects anonymous access, and each supported role has a tested allow/deny case.
